<script>
        ...
        Authentication.usernameFromCode(username, token)
            .then(
                ({
                    ...
                    ProjectApi.getProfile(username, false, token).then(
                        (profile) => {
                    ...
                    if (
                        ...
                        !permittedPages.includes(location.pathname)
                    ) {
    ...
    function switchTheme() {
        ...
        const hasDarkOption =
            darkThemeOption !== null && darkThemeOption !== undefined;
        if (
            String(darkThemeOption) === "true" ||
            (!hasDarkOption && prefersDarkMode)
        ) {
    ...
    onMount(() => {
        window.addEventListener("mousedown", (e) => {
            ...
            if (accountMenu && accountButton) {
                if (
                    !HTMLUtility.isDescendantOf(accountMenu, e.target) &&
                    !HTMLUtility.isDescendantOf(accountButton, e.target)
                ) {
                    accountMenu.style.display = "none";
                }
            }
        ...
        >
            <div class="languageProgress">
                <div class="only-in-dark-mode">
                    <CircularProgress
                        progress={Translations.getLanguageFinishedPercentage(
                            languageCode,
                        )}
                        holeColor="#222"
                        emptyColor="#555"
                ...
                <span>
                    {Math.round(
                        Translations.getLanguageFinishedPercentage(
                            languageCode,
                        ) * 100,
                    )}
                </span>
...
<style>
    :root {
        --penguinmod-color: #a3b493;
    }
    :global(body.dark-mode) {
        --penguinmod-color: #8a9b7a;
    }
